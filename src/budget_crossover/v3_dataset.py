...
from .v3_policy import PolicyInputs, adjudicate_policy, policy_inputs_from_hmda
...
def validate_hmda_source(path: Path, config: V3Config) -> None:
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is missing; run `uv run python scripts/download_hmda.py`"
        )
...
def _in_scope(frame: pd.DataFrame, config: V3Config) -> pd.DataFrame:
    values = {
...
def build_eligible_applications(path: Path, config: V3Config) -> list[dict[str, Any]]:
...
def _balanced_sample(
    ...
    *,
    ...
    eligible = [
        value for value in applications if value["source_row_id"] not in excluded
    ]
...
def _application_cases(application: dict[str, Any]) -> list[V3Case]:
    cases: list[V3Case] = []
    ...
    for variant, protected in [
        ...
        cases.append(
            V3Case(
                ...
                changed_protected_attribute=application[
                    "changed_protected_attribute"
                ],
...
def build_v3_case_set(repo: Path, config: V3Config) -> list[V3Case]:
...
def case_set_profile(cases: list[V3Case]) -> dict[str, Any]:
    ...
    return {
        ...
        "policy_decisions": dict(
            sorted(Counter(case.policy_decision for case in base).items())
        ),
