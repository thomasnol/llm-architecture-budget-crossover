...
from .v3_policy import POLICY_TEXT, canonical_decision
...
class BudgetedCaller:
    ...
    def _observed_total(self, response: GatewayResponse, prompt_estimate: int) -> int:
        ...
        completion_estimate = math.ceil(
            len(response.text) / self.config.prompt_chars_per_token
        )
    ...
    async def call(self, request: CallRequest) -> CallRecord:
        ...
        prompt_estimate, output_tokens = self._allocation(
    ...
    async def call_parallel(self, requests: list[CallRequest]) -> list[CallRecord]:
        ...
        remaining = self.token_budget - self.accounted_tokens
        prompt_estimates = [
            self.estimate_prompt_tokens(request.system, request.user)
            for request in requests
        ]
        minimum = sum(prompt_estimates) + len(requests) * (
            self.config.minimum_call_output_tokens
        )
        ...
        allocations = [
            min(request.desired_output_tokens, equal_share) for request in requests
        ]
...
def _documents(case: V3Case, names: list[str]) -> str:
...
def _call_request(
    ...
    *,
    ...
    return CallRequest(
        ...
        user=user,
        temperature=(
            config.specialist_temperature
            if specialist
            else config.direct_temperature
        ),
...
async def run_v3_system(
    ...
    *,
    ...
    final_text = ""
    try:
        if system in {"monolith", "strong_monolith"}:
            evidence = _documents(
                case,
                [
                    "application",
...
{OUTPUT_INSTRUCTION}""",
        ...
        elif system in {"guardrail", "adaptive"}:
            financial = _documents(
                case, ["application", "collateral", "credit", "quality_control"]
            )
            ...
            diagnostics["route_signals"] = {
                ...
                "non_approve_draft": (
                    draft_decision is not None
                    and draft_decision.get("decision") != "approve"
                ),
