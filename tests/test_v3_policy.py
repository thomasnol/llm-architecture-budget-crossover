from budget_crossover.v3_policy import PolicyInputs, adjudicate_policy, dti_category
...
def test_policy_precedence_and_reason_codes():
    ...
    assert adjudicate_policy(
        _inputs(debt_to_income_band="45", loan_to_value_percent=95)
    ) == ("conditional_review", ["elevated_dti", "elevated_ltv"])
    assert adjudicate_policy(
        _inputs(
    ...
    assert adjudicate_policy(
        _inputs(annual_income_usd=None, debt_to_income_band=None)
    ) == ("manual_review", ["missing_dti", "missing_income"])
