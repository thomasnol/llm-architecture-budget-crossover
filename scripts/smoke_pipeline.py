...
from budget_crossover.analysis import analyze_run
from budget_crossover.config import ExperimentConfig, load_experiment_config
from budget_crossover.dataset import build_case_set
...
from budget_crossover.validation import validate_run
...
class ScriptedGateway:
    ...
    async def complete(self, **kwargs: Any) -> GatewayResponse:
        ...
        if '"request":["tool_name"' in user:
            ...
            text = self._decision()
        prompt_tokens = math.ceil((len(str(kwargs["system"])) + len(user)) / 4.0) + 8
        ...
        return GatewayResponse(
            ...
            credential_slot=(1 if str(kwargs["model"]).startswith("claude") else 2),
            request_id=f"offline-{len(self.calls)}",
            raw_finish_reason="stop",
        )


async def _generate(config: ExperimentConfig, cases: list[Case]) -> list[Generation]:
            ...
            for budget in config.token_budgets:
                for repetition in range(config.repetitions):
                    ...
                    result = await run_system(
                        ...
                        config=config,
                        run_id=(
                            f"{config.experiment_name}-{case.case_id}-{system}-"
                            f"b{budget}-r{repetition}"
                        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Exercise the canonical orchestration and analysis pipeline offline."
    )
    parser.add_argument(
        "--keep-existing",
        action="store_true",
    ...
    output_dir = run_dir(REPO, config)
    if output_dir.exists():
        if args.keep_existing:
            raise SystemExit(f"smoke run already exists: {output_dir}")
        shutil.rmtree(output_dir)

    cases = build_case_set(REPO, config)
    ensure_manifest(REPO, config, cases)
    generations = asyncio.run(_generate(config, cases))
    write_jsonl(generation_path(REPO, config), generations)
    report = analyze_run(repo=REPO, config=config, cases=cases)
    ...
    (output_dir / "analysis" / "analysis.json").write_text(
        json.dumps(report, indent=2, sort_keys=True)
    )
    if not validation["pass"]:
        raise SystemExit("offline smoke validation failed: " + "; ".join(validation["issues"]))
